...
qbVector<double> sempRT::Image::GetColor(const qbVector<double> &uvCoords)
{
...
bool sempRT::Image::LoadImage(std::string fileName)
{
	if (m_imageLoaded)
		SDL_FreeSurface(m_imageSurface);
...
void sempRT::Image::GetPixelValue(int x, int y, double &red, double &green, double &blue, double &alpha)
{
	if ((x >= 0) && (x < m_xSize) && (y >= 0) && (y < m_ySize))
	{	
...
double sempRT::Image::LinearInterp(const double &x0, const double &y0, const double &x1, const double &y1, const double &x)
{
	double output;
...
double sempRT::Image::BilinearInterp(const double &x0, const double &y0, const double &v0,
